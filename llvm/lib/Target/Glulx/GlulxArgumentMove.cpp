//===-- GlulxArgumentMove.cpp - Argument instruction moving ---------===//
//
...
#define DEBUG_TYPE "Glulx-argument-move"

namespace {
class GlulxArgumentMove final : public MachineFunctionPass {
public:
  static char ID; // Pass identification, replacement for typeid
  GlulxArgumentMove() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "Glulx Argument Move"; }
...
char GlulxArgumentMove::ID = 0;
INITIALIZE_PASS(GlulxArgumentMove, DEBUG_TYPE,
                "Move ARGUMENT instructions for Glulx", false, false)

FunctionPass *llvm::createGlulxArgumentMove() {
  return new GlulxArgumentMove();
}

bool GlulxArgumentMove::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG({
    dbgs() << "********** Argument Move **********\n"
           << "********** Function: " << MF.getName() << '\n';
  });
