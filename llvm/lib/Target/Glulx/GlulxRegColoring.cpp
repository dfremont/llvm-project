//===-- GlulxRegColoring.cpp - Register coloring --------------------===//
//
...
///
/// Glulx doesn't have a fixed number of locals, but it is still
/// desirable to minimize the total number of locals used in each function.
///
...
#define DEBUG_TYPE "glulx-reg-coloring"

namespace {
class GlulxRegColoring final : public MachineFunctionPass {
public:
  static char ID; // Pass identification, replacement for typeid
  GlulxRegColoring() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override {
    return "Glulx Register Coloring";
  }
...
char GlulxRegColoring::ID = 0;
INITIALIZE_PASS(GlulxRegColoring, DEBUG_TYPE,
                "Minimize number of locals used", false, false)

FunctionPass *llvm::createGlulxRegColoring() {
  return new GlulxRegColoring();
}
...
bool GlulxRegColoring::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG({
  ...
  SmallVector<Register, 16> SlotMapping(SortedIntervals.size(), -1u);
